...
def verify_c_extension_version():
    """Check if loaded C extension matches app version"""
    try:
        # If your C extension exports a version function
        lib = get_c_library()
        if hasattr(lib, 'c_hash_version'):
            c_ver = lib.c_hash_version().decode()
            if c_ver != C_VERSION:
                print(f"[c] Warning: C extension v{c_ver} != app v{C_VERSION}")
                return False
    except:
        pass
    return True
